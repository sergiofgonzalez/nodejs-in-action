...
```bash
sam-lambda-health-check$ sam build
```
...
```bash
sam-lambda-health-check$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-lambda-health-check$ sam local start-api
sam-lambda-health-check$ curl http://localhost:3000/
```
...
```bash
sam-lambda-health-check$ sam logs -n HelloWorldFunction --stack-name sam-lambda-health-check --tail
```
...
```bash
sam-lambda-health-check$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name sam-lambda-health-check
```
